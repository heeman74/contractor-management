...
export default function ForemanError({ error, reset }: ErrorProps) {
  useEffect(() => {
    logger.error("ForemanError", error.message, {
      name: error.name,
      digest: error.digest,
      stack: error.stack,
    });
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-4 rounded-xl border border-red-200 bg-red-50 px-6 py-12 text-center">
      ...
      <p className="max-w-md text-sm text-red-600">
        The foreman section encountered an unexpected error. You can try
