...
- [ ] **COST-01**: Owner/PM can record a materials cost entry (amount, category, date, vendor, note) against a job or trade scope
- [ ] **COST-02**: Owner/PM can record subcontractor and other cost entries the same way
- [ ] **COST-03**: Owner/PM can attach a receipt photo to a cost entry
- [ ] **COST-04**: Owner/PM can set a worker's hourly cost rate with an effective date; historical rates are preserved
- [ ] **COST-05**: System derives labor cost automatically from tracked time × the rate effective on the day worked
...
- [ ] **MARG-01**: Owner/PM can see profit margin (revenue − actual costs) per job/trade scope
- [ ] **MARG-02**: Owner/PM can see project-level margin rollup across trades
- [ ] **MARG-03**: Margin views flag incomplete cost data (legacy jobs, missing rates) instead of showing misleading numbers
...
- [ ] **FINAI-02**: Owner/PM receives finance-gated alerts for AI profitability findings
...
- [ ] **FINSEC-01**: All financial endpoints are backend-gated by finance.* permissions, granted only to owner and project_manager by default
- [ ] **FINSEC-02**: Companies can adjust finance.* grants via the existing Roles & Permissions matrix
- [ ] **FINSEC-03**: The admin role does not inherit finance.* (explicit exclusion from the derived permission set)
